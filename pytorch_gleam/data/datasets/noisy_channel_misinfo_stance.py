...
class NoisyChannelMisinfoStanceDataModule(BaseDataModule):
    def __init__(
        ...
        if self.val_path is not None:
            self.val_dataset = NoisyChannelMisinfoStanceDataset(
                data_path=self.val_path,
                misinfo_path=self.misinfo_path,
                preprocess_config=preprocess_config,
            )
        if self.test_path is not None:
            self.test_dataset = NoisyChannelMisinfoStanceDataset(
                data_path=self.test_path,
                misinfo_path=self.misinfo_path,
                preprocess_config=preprocess_config,
            )
        if self.predict_path is not None:
            self.predict_dataset = NoisyChannelMisinfoStanceDataset(
