...
from pytorch_gleam.data.collators import DirectACSBatchCollator
...
class DirectACSMisinfoStanceDataset(MisinfoStanceDataset):
    def __init__(self, pos_samples: int = 1, *args, **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        labels = [tm_stance] + [p_ex["m_label"] for p_ex in pos_samples]
        stages = [t_ex["stage"]] + [p_ex["stage"] for p_ex in pos_samples]
        relations = [tmp_relation for _ in range(len(pos_samples))]
        directions = [direction for _ in range(len(pos_samples))]
...
class DirectACSInferStanceDataset(DirectACSMisinfoStanceDataset):
...
class DirectACSMisinfoStanceDataModule(BaseDataModule):
    def __init__(
        self,
        misinfo_path: str,
        tokenizer_name: str,
        ...
        **kwargs
        ...
        if self.train_path is not None:
            self.train_dataset = DirectACSMisinfoStanceDataset(
        ...
        if self.val_path is not None:
            val_triplet_dataset = DirectACSMisinfoStanceDataset(
                pos_samples=self.pos_samples,
                data_path=self.val_path,
                misinfo_path=self.misinfo_path,
                preprocess_config=preprocess_config,
            )
            val_infer_dataset = DirectACSInferStanceDataset(
                pos_samples=1,
                data_path=self.val_path,
                misinfo_path=self.misinfo_path,
                preprocess_config=preprocess_config,
        ...
        if self.test_path is not None:
            test_triplet_dataset = DirectACSMisinfoStanceDataset(
                pos_samples=self.pos_samples,
                data_path=self.test_path,
                misinfo_path=self.misinfo_path,
                preprocess_config=preprocess_config,
            )
            test_infer_dataset = DirectACSInferStanceDataset(
                pos_samples=1,
        ...
        if self.predict_path is not None:
            self.predict_dataset = DirectACSInferStanceDataset(
                pos_samples=1,
