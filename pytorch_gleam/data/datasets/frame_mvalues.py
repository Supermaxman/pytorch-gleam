...
from pytorch_gleam.data.collators import MultiClassFrameMultiValuesBatchCollator
from pytorch_gleam.data.datasets.base_datasets import BaseDataModule
from pytorch_gleam.data.twitter import preprocess_tweet, read_jsonl, TweetPreprocessConfig


class MultiClassFrameMultiValuesDataset(Dataset):
    ...
    def __init__(
        ...
        self.cultural_values_map = {v: i for i, v in enumerate(self.cultural_list)}
        self.moral_values_map = {v: i for i, v in enumerate(self.moral_list)}
        ...
        for f_id, frame in self.frames.items():
            ...
            frame["cultural_value_ids"] = []
            frame["moral_value_ids"] = []
            for val, _ in frame["values"].items():
                if val in self.cultural_values_map:
                    frame["cultural_value_ids"].append(self.cultural_values_map[val])
                elif self.skip_unknown_labels:
                    continue
                else:
                    raise ValueError(f"Unknown value: {val}")
            for val, _ in frame["moralities"].items():
                if val in self.moral_values_map:
                    frame["moral_value_ids"].append(self.moral_values_map[val])
                elif self.skip_unknown_labels:
                    continue
                else:
                    raise ValueError(f"Unknown value: {val}")
            ...
            if self.all_frames:
                for f_id, frame in self.frames.items():
                    ...
                    example = {
                        ...
                        "cultural_value_ids": frame["cultural_value_ids"],
                        "moral_value_ids": frame["moral_value_ids"],
                        "gold": gold,
                    }
...
class MultiClassFrameMultiValuesDataModule(BaseDataModule):
    def __init__(
        self,
        label_name: str,
        label_map: Dict[str, int],
        ...
        **kwargs,
        ...
        if self.train_path is not None:
            self.train_dataset = MultiClassFrameMultiValuesDataset(
                data_path=self.train_path,
                frame_path=self.frame_path,
                label_name=self.label_name,
                label_map=self.label_map,
                cultural_list=self.cultural_list,
                moral_list=self.moral_list,
        ...
        if self.val_path is not None:
            self.val_dataset = MultiClassFrameMultiValuesDataset(
        ...
        if self.test_path is not None:
            self.test_dataset = MultiClassFrameMultiValuesDataset(
        ...
        if self.predict_path is not None:
            self.predict_dataset = MultiClassFrameMultiValuesDataset(
    ...
    def create_collator(self):
        return MultiClassFrameMultiValuesBatchCollator(
            self.tokenizer,
            self.cultural_list,
            self.moral_list,
