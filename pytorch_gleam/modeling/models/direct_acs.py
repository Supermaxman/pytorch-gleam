...
class DirectACSLanguageModel(BasePreModel):
    def __init__(
        self,
        pre_model_name: str,
    ...
    def eval_infer(
        ...
        # stage 1 is test
        m_adj_lists, m_stage_labels = DirectACSLanguageModel.build_adj_list(infer_eval_outputs)
        ...
        for m_id, stage_labels in m_stage_labels.items():
            ...
            m_ex_scores = DirectACSLanguageModel.infer_m_scores(infer, m_adj_list, stage_labels, stage, num_val_seeds)
            if update_threshold:
    ...
    def build_adj_list(outputs):
        # [count]
        t_ids = DirectACSLanguageModel.flatten([x["ids"] for x in outputs])
        # [count]
        m_ids = DirectACSLanguageModel.flatten([x["m_ids"] for x in outputs])
        # [count]
        # skip every other s_id since they are duplicates from two relationships
        p_ids = DirectACSLanguageModel.flatten([x["s_ids"] for x in outputs])[::2]
