...
class MultiValuesModule(nn.Module, ABC):
    def __init__(
        ...
        self.value_list = value_list
        self.num_values = len(value_list)
        self.cultural_embeddings = nn.Embedding(self.num_values, self.output_dim)
        self.moral_embeddings = nn.Embedding(self.num_values, self.output_dim)
...
class MultiValuesWeightedAttentionPooling(MultiValuesModule):
    ...
    def forward(self, embeddings, embeddings_mask, cultural_mask, moral_mask, post_mask, frame_mask):
        ...
        # value attention
        cultural_embedding, cultural_probs = self.value_attention(
            embeddings, embeddings_mask, cultural_mask, self.cultural_embeddings
        )
        moral_embeddings, moral_probs = self.value_attention(
            embeddings, embeddings_mask, moral_mask, self.moral_embeddings
        )

        # attention pooling
        post_embedding, post_probs = self.attention_pooling(
            embeddings, post_mask, (cultural_embedding + moral_embeddings) / 2.0
        )

        frame_embedding, frame_probs = self.attention_pooling(
            embeddings, frame_mask, (cultural_embedding + moral_embeddings) / 2.0
        )
        ...
        return {
            ...
            "pooled_embedding": pooled_embedding,
            "cultural_probs": cultural_probs,
            "moral_probs": moral_probs,
            "post_probs": post_probs,
            "frame_probs": frame_probs,
            "output_features": pooled_embedding,
        }
