...
def update_status(queue_path, ex, status, p_id=None):
    old_file_path = os.path.join(
        queue_path, ex["current_status"]["status"], ex["ex_id"]
    )
        ...
        with FileLock(os.path.join(queue_path, ".lock")):
            ...
            for ts, ex in ex_queue["running"]:
                ...
                if p is None:
                    ...
                    update_status(
                        queue_path=queue_path, ex=ex, status="submitted", p_id=None
                    )
                else:
                    ret = p.poll()
                    if ret is None:
                        ...
                        update_status(
                            queue_path=queue_path, ex=ex, status="completed", p_id=None
                        )
                        processes[p_id] = None
            ...
            if num_to_run > 0:
                top_to_run = heapq.nsmallest(
                    num_to_run, available_to_run_list, key=lambda x: x[0]
                )
                for ts, ex in top_to_run:
                    ...
                    update_status(
                        queue_path=queue_path, ex=ex, status="running", p_id=p_id
                    )
